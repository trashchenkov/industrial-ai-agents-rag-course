...
`01_ingestion_pipeline.ipynb` собирает учебный корпус из открытых русскоязычных источников:
...
Затем создайте окружение и установите зависимости. Рекомендуемая версия — Python 3.12. Python 3.11 тоже подойдёт, но версии старее использовать не стоит. Python 3.13, как правило, тоже должен подойти, но если какая-то зависимость ещё не поддерживает 3.13, проще вернуться на 3.12. Если команда `python3.12` не найдена, установите Python 3.12 или замените её на команду, которая в вашей системе запускает подходящую версию Python, например `python3.11`, `python3.13` или `py -3.11`.

Проверить, какие версии Python доступны на компьютере:
